...
- **RaisedKey press grammar**: every committing/navigating button is an opaque
  ...
  the escape read as the thing to do. On `RestView` that was Skip, which was
  also the widest key and the one under the thumb. The rest screen's only
  filled keys are now the two that ADJUST the countdown; Skip is a quiet cap,
  set apart from them by an extra gap so a thumb stepping `−15s` twice can't
  end the rest. Where an escape shares a row with keys you tap repeatedly,
  give every key the SAME travel (`.raisedKey` beside `.raisedPrimaryKey`,
  not `.quietKey`'s 3 pt) so the caps sit on one baseline, and carry the
  quiet reading in the cap's fill, ink and type instead.
...
⚠️ **This section describes the app as it IS. It is not a changelog** — the
round-by-round history that used to live here (design v3/v4, the build-27 and
build-28/29 feedback rounds, the 2026-07-17 equipment-catalog rebuild) is in
docs/DECISIONS.md under its dated entries, and was removed on 2026-07-28 after
it had drifted into stating things that are no longer true. Correct a claim
here when the app changes; don't append a new dated paragraph beside it.
...
container laws that constrain all of this (the `.principal` toolbar row,
search scopes, the accessory's retirement, the morph's state-write rule) are
above in this file; read them before changing anything in that row.
...
Pull-to-refresh answers in the gap the pull opens.
