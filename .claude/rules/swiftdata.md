...
**`fullScreenCover(item:)` / `persistentModelID`:** the ID CHANGES at the first save of a fresh model — save synchronously at creation (`WorkoutSession.start`) so a live presentation never re-keys.
