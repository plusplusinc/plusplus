...
**Adding a field prefers lightweight + a launch backfill, NOT a custom stage with snapshot classes (hard-won, 2026-07-14):** an OPTIONAL additive property migrates in automatically — existing rows get `nil` — and a content-keyed, idempotent launch pass (`SeedData.backfillModelUUIDsIfNeeded`, from `PlusPlusApp.init`) populates them. A NON-optional attribute has no static default → the column-add fails validation on existing rows (`NSCocoaError 1560`). ⚠️ **Do NOT reach for a frozen `VersionedSchema` snapshot + custom `didMigrate` for this:** nested snapshot `@Model` classes that share an entity name with the live top-level classes crash migration with a fatal `Failed to cast model AppSchemaV1.X to X`. Only add a new `VersionedSchema` + `MigrationStage` when a change genuinely can't ride lightweight — and expect that hazard.

**⚠️ A `= UUID()` (or any non-constant) PROPERTY-LEVEL default is applied by lightweight migration as ONE SHARED CONSTANT across every existing row (hard-won, 2026-07-14, caught only by pre-merge on-device validation):** it shipped a store where every `Routine` shared one `uuid`, so nav resolved every routine to the same one and `ForEach(id: \.uuid)` drew duplicate rows. Set per-instance identity in `init` (`self.uuid = UUID()`), leave the stored property defaultless (`var uuid: UUID?`) so migrated rows are `nil`, and make the backfill **enforce uniqueness** (reassign nil OR duplicate) so it also repairs a store already stamped with the shared default. Migration correctness is **on-device-only** — simulator/CI green ≠ production safety, AND the interchange export can be byte-identical while identity is silently broken (uuid is device-local, excluded from export), so a data-diff does NOT prove the migration is correct; you must open the app and navigate.
