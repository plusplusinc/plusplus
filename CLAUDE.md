...
**Branch protection is active** (repository ruleset): merges to main require the `test` status check to PASS on the head SHA. Consequences: a cancelled required check blocks merge until re-run, and **docs-only PRs never trigger `test`** (ci.yml has `paths-ignore: **.md, docs/**`) — dispatch ci.yml on the branch manually (`workflow_dispatch`) to satisfy the check before merging.
...
**TestFlight:** `.github/workflows/testflight.yml` (manual dispatch, any ref) archives unsigned, cloud-signs at export with an Admin-role ASC API key, and uploads; build number = run number. **Build 17** is current: platform batch 1 (#147 → PR #152 — Live Activity, widgets, App Intents) plus the open-source licensing (#154). Build 16 = store-recovery fix (#153: build 15 crash-looped on update-in-place installs because the #144 entity renames made old stores unreadable; the app now destroys-and-recreates on open failure — beta stopgap, 1.0 policy is #155). Build 15 = routines rename (#144) + share links (#145). ⚠️ Update-in-place is safe from build 16 onward; 15 needed a fresh install. Build 17 was the first export to exercise **App Group entitlements under cloud signing** — it succeeded with no developer-portal changes (cloud signing provisioned the capability itself), and org Actions secrets survived the transfer.
...
- 2026-07-07 batch (#155–#165): store-migration policy for 1.0 (#155), universal links (#156), Live Activity controls (#157), platform batch 2 — Control Center/Spotlight/complication (#158), widget snapshot freshness (#159), contribution infrastructure for the public repo (#160), org-transfer cleanup (#161), diff share cards + contribution graph (#162), README streak-badge recipe (#163), accessibility completion (#164), Foundation Models importer (#165).
...
**2026-07-07 (late eve) — Branch protection: required `test` check on main** — A repository ruleset now gates merges on the `test` (macOS) job passing. Two operational consequences: cancelled required checks must be re-run before merge, and docs-only changes (paths-ignored by ci.yml) need a manual `workflow_dispatch` of ci.yml on the branch to produce the check. The Claude GitHub App needed installing on the plusplusinc org for API merges to work at all.
