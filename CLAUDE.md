...
- **PlusPlusTests** — unit test target (22 tests)
...
PlusPlusTests/
  ...
  ReindexTests.swift         # Reindex helper tests (5 tests + 1 placeholder = 22 total)
...
**Known TODOs:**
- Weight/reps/duration inputs use basic TextFields with `.number` format — janky cursor behavior. Replace with custom input controls (scroll/stepper) in a future chunk.
- Superset creation UI — the data model supports supersets (multiple exercises in one ExerciseGroup), but there's no UI to create them yet. Each exercise currently gets its own group.
- No workout execution (WorkoutSession) — that's a future chunk.
