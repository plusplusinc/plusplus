...
**Last updated:** 2026-08-05 · **Latest TestFlight build:** 187, from `claude/search-scopes-control-replace-2z9m5x` (#553, native search scopes + the field moved to the top) — NOT main, and deliberately so: both are device-passed BEFORE they land. ⚠️ 170–185 came from parallel sessions and were never recorded here (this line said 169 for three days); 185 was main at `5e94130`, 181–184 feature branches. Read `actions_list`, never this line, for the CURRENT number.
...
**In flight:** #553 — TWO unvalidated search-surface changes, as separate commits so either reverts alone. (1) the scope control becomes native `.searchScopes` and the app-drawn `ScopeSegmentedControl` is deleted (another shot at the mechanism retired at builds 140–143). (2) the FIELD moves to the top (`.navigationBarDrawer(displayMode: .always)`), which cost `Tab(role: .search)` — that role's morph IS the bottom placement (Dave, 2026-08-05). ⚠️ Both can fail outright rather than merely look wrong, which is why builds 186/187 ship from the branch: the revert is a branch that never merged. `LiveWorkoutSettings` (phone's own `HKWorkoutSession`) remains off by default.
