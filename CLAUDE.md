...
**Last updated:** 2026-08-01 · **Latest TestFlight build:** 159, from `claude/plusplus-cardio-workouts-whh64j` (the cardio push + follow-ons, cut so #482 could stay open; builds before 158 came from main).
...
- Design-review round on #482 — ONE facet grammar (main's `FacetChip` row wins; KIND · Cardio/Strength/Mobility leads the exercises facets, `CatalogFacetRow` deleted, main's duplicate Swimming seed row yields to the swim pair), a finished session's noun is a SNAPSHOT (`summaryWorkUnit`, stamped at finish — history and the finish screen can no longer disagree on a mixed session), the count-up clock anchors on the session's ledger (`effortAnchorSeconds` — survives pause AND process death, rest-end stamps win), `isSingleEffort` is cardio-gated (one bench set no longer auto-finishes), overview rows obey count-of-one, quick-start picks follow renames; reasoning in docs/DECISIONS.md (latest)
...
- Build-158 feedback — ONE indoor-bike row (the activity noun wins; the equipment keeps "Stationary Bike"), quick start moved out of the start tray onto Today's rail under the date, and the week strip's reserved space draws the SPINE; ⚠️ dropping a seed definition never removes a row a store already has, so `mergeIndoorBikeExercises` RENAMES the old one (a rename keeps the object, so routines and logs come along) and only deletes a duplicate nothing references
- A cardio record derives its own pace from the distance and the time it logged (#302's second half; an entered pace still wins); ⚠️ once a value can be COMPUTED, every `actual(x) == nil` guard changes meaning — `actual(_:)` shows a fact, `storedActual(_:)` decides whether to write one, and the three write paths that had to move are the GPS write-back, `complete`'s prefill and `saveAsRoutine`
- Cardio push (#472–#481, build 158) — a cardio effort has a HERO now (`CardioHero`: progress toward a target the device can measure RIGHT NOW → a target only the console can show you → the best live reading → elapsed counting up), an untargeted one gets a count-up clock where it had none, and quick start puts a sport one tap behind Today's play key; ⚠️ the count-up ANCHOR is `session.effortAnchorSeconds` on the running-time ledger (pausing UNMOUNTS the timer card and in that mode the displayed elapsed IS the logged duration, so the anchor must outlive every view)
...
**In flight:** `claude/plusplus-cardio-workouts-whh64j` — the cardio integration branch, one PR into main (#482), awaiting Dave's device pass. PR 10 landed: the phone runs its own `HKWorkoutSession` behind `LiveWorkoutSettings`, off by default.
...
**CI flakes:** ui-test has two known flavors — `app.launch()` wedging on a runner simulator, and exit-65 runs where the identical tree passes on re-run. Re-run once before suspecting code. All four jobs surface failing-test names as `::error::` annotations readable via the check-runs API. ⚠️ Job LOGS are reachable after all (2026-07-30): `mcp__github__get_job_logs` with `return_content: true` serves them through the API, so only the ARTIFACTS are genuinely out of reach from a sandbox.
