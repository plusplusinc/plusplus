...
- Quick start lives INSIDE Today's sticky week-strip band (an OFFER is not an occurrence, and round 2 said anything in the scroll flow still reads as timeline) with IMPERATIVE keys (`QuickStartLabel`: Run · Ride · Spin · Swim…, colliding picks fall back to names); ⚠️ device pass: keys must HIT where they draw mid-float (first interactive content in the visualEffect band — fallback is the below-band row), plus the "Spin"/"Swim open water" judgment calls (Dave, build 159 ×2)
- Design-review round on #482 — ONE facet grammar (KIND leads the exercises facets; `CatalogFacetRow` deleted, one Swimming row), a finished session's noun is a SNAPSHOT (`summaryWorkUnit`), the count-up clock anchors on the session's ledger (`effortAnchorSeconds` — survives pause AND process death), `isSingleEffort` is cardio-gated, count-of-one holds everywhere, picks follow renames; detail in docs/DECISIONS.md
...
- Build-158 feedback — ONE indoor-bike row (the activity noun wins; the equipment keeps "Stationary Bike") and the week strip's reserved space draws the SPINE; ⚠️ dropping a seed definition never removes a row a store already has, so `mergeIndoorBikeExercises` RENAMES the old one (a rename keeps the object, so routines and logs come along) and only deletes a duplicate nothing references
...
**In flight:** nothing — #482 (the cardio push + review round) is MERGED; the device pass needs a fresh build (159 predates the review round).
