...
⚠️ **`ui-test` has been RED on main since 2026-07-24** — 22 consecutive push runs, not the four this line first claimed (that was the window I had looked at; last green is `337c3397`). It is NOT the cardio push and NOT a required check, which is why it went unnoticed through both build 157 and build 158. ⚠️ It is also NOT the #273/#274 swipe regression this line first suspected: the swipe test fails at SmokeTests:144, the `backButton` assertion in its SETUP, before any drag. Three unrelated causes, all of them the SUITE drifting from the app — #445 moved the schedule mode off a segment, #452 changed the search surface, #470 took routine detail off `pushedScreenChrome`. Fix + the process gap: #483 / PR #484.
