...
- **Plugin** — `axiom@axiom-marketplace` (Apple-platform skills + agents; marketplace `CharlesWiltgen/Axiom`, declared in `.claude/settings.json`). Its **knowledge skills** (`axiom-health` for HealthKit, `axiom-swift`, `axiom-concurrency`, `axiom-swiftui`, …) and **reasoning agents** (auditors like `security-privacy-scanner`) DO load and help in remote Linux sessions — reach for them on Apple-platform work (`axiom-health` was used to validate the HealthKit sync work). Only its Xcode/Simulator-backed tooling (build/run/screenshot/simulator) is inert remotely. ⚠️ An ephemeral remote container does NOT auto-install it from `enabledPlugins`; the `SessionStart` hook `.claude/hooks/ensure-axiom.sh` installs it each session (and the install bakes into the cached container). Treat its keyword-triggered hook matches as advisory. See docs/DECISIONS.md 2026-07-13 (Axiom).
