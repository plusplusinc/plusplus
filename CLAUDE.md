...
- Today's header band (QUICK START keys · tally · bar) is PINNED CHROME — a top safeAreaInset, NOT scroll content (reverses the sticky law; float/reservation machinery deleted), keys IMPERATIVE (`QuickStartLabel`) "N more" overflow, no horizontal scroll, and the start tray leads with a primed today key + the SAME rack (one system, two mouths); ⚠️ device pass: the PULL (build 152's ghost, accepted — fallback 8b9e16b), plus "Spin"/"Swim open water" (Dave ×6)
