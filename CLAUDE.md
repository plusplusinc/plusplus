...
**Last updated:** 2026-07-11. **Build 48** dispatching from PR #312 (equipment libraries: per-location gear as a named `EquipmentLibrary`, one active at a time, tray switcher, own→have language sweep, libraries + gear config in the interchange for new-phone restore). Over build 47 (Today-details UX #306 + welcome refinement #310) and build 46 (flexible metrics #304 + heart rate #297). ⚠️ Build number = testflight RUN number — 48 is the target; confirm the dispatched run's number and relabel the What's-New entry if it differs. Update-in-place is safe from build 16 onward. plusplus.fit LIVE; universal links ON.
