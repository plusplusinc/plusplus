...
- Record round (#506): history carries MONTH landmarks (pinned `Section` headers — ⚠️ NOT the issue's stale sticky-`visualEffect` hint; laws in navigation.md), each completed set prints a faint `asked:` reference in FLAT ink (ALWAYS when a target exists — a line that appeared only on a miss would BE the judgment), and a run states its headline once (a lone outdoor block drops; intervals stay); ⚠️ device pass: header hand-off, run wording
