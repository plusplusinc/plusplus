...
- **Rules** (`.claude/rules/`) — path-scoped patterns: `swiftdata.md`, `testing.md`, `ui-interaction.md`, `app-surfaces.md` (surface map), `design-grammar.md` (color/keys/tags/motion/copy laws), `navigation.md` (surface list/search/scroll laws). They load when you read matching files; skim them anyway before big app work.
...
Split into path-scoped rules in `.claude/rules/` (they auto-load when you touch matching files): `swiftdata.md` (container/relationship laws), `testing.md` (test isolation + red-first bug fixes, XCUITest blind spots), `ui-interaction.md` (swipe/gesture laws), `design-grammar.md` (color/keys/tags/motion/copy), `navigation.md` (surface list/search/scroll), `app-surfaces.md` (surface map). Add new patterns to the matching rule file — or a new one — not here.
