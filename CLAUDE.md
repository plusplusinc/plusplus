...
- Catalog round (#507): a facet that hides a query MATCH says so ("N more … hidden by filters · show" ABOVE the create row — the create row is the near-duplicate path) and a hand-built routine under Effort/Style groups as "not rated", never vanishes; ⚠️ the hidden count rides `FindOrCreateEngine.outcome`'s ONE pass (score first, facet second) — deriving it from an unfiltered pass is the retired second-ranking-pass cost; all three create rows read `Create "<query>"` now; b10 HELD BACK (device-verify the overflow); ⚠️ navigation.md has 79 bytes left — the next law there SPLITS the file
