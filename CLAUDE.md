...
- One effort is not a repetition: on a session that is a single continuous effort the commit key says "Finish workout" and ENDS it (`WorkoutSession.isSingleEffort`), instead of logging and then asking "add another, or finish"; ⚠️ it keys on the SESSION (a run then core work must advance, not finish), it is live so adding an exercise takes the ending back, and the async finish RE-READS it rather than trusting log time
