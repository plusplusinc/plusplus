...
- **Plugin — `axiom@axiom-marketplace`** (project scope): a large library of Apple-platform skills + auditor agents (accessibility, SwiftUI layout/nav/architecture, concurrency, performance, memory, Core Data / SwiftData, testing, crashes, HIG/design/typography, shipping…). **Reach for it whenever a task falls in its wheelhouse** — before hand-rolling an audit or re-deriving Apple guidance, check for a matching Axiom skill/agent and use it (e.g. accessibility work → `accessibility-auditor` + `axiom-accessibility`; Dynamic Type / layout → `swiftui-layout-auditor` + `axiom-design`'s typography/hig refs; a review → the relevant `*-auditor`). The auditor agents are pure static Glob/Grep/Read analysis, so **they run fine in remote (Linux) sessions** even though Axiom's macOS runtime tools (`xcui`/`xclog`/`xcsym`/`xcprof`, `test-simulator`, `/axiom:*` device commands) stay inert here — if the plugin's agents aren't registered in this session yet (installed mid-session), run the auditor by reading its definition under `~/.claude/plugins/cache/axiom-marketplace/…/agents/<name>.md` and executing that methodology via a subagent. Treat its keyword-triggered hook matches as advisory.
