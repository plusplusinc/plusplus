...
- Today's rail is DATE-FIRST (dates out of cards, node centered on the date row) and quick start is its ANYTIME entry — dashed node + dashed-shell `AnytimeCard`, keys MORPH in place into config panels (`matchedGeometryEffect`, CHROME only — laws in navigation.md), Train = Start empty / Choose a routine, band = FACTS only; ⚠️ device pass: morph feel, offer-above-plan, the PULL (152's ghost — fallback 8b9e16b), "Spin"/"Swim open water"
