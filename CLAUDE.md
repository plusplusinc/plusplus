...
- Today's header band (QUICK START keys · tally · bar) is PINNED CHROME — a top safeAreaInset, NOT scroll content (reverses the sticky law), keys IMPERATIVE (`QuickStartLabel`) with "N more" overflow, and the PLAY KEY + START TRAY are DELETED — "Train" leads the rack (the scratch start), routines start from their cards/tab; ⚠️ device pass: the PULL (build 152's ghost, accepted — fallback 8b9e16b), plus "Spin"/"Swim open water" (Dave ×7)
...
**In flight:** #502 — Today's quick-start design loop (pinned band → play-key kill); merges on Dave's word. The device pass needs a fresh build (159 predates it all).
