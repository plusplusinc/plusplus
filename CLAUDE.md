...
- Record round (#506): history carries MONTH landmarks (pinned `Section` headers — ⚠️ NOT the issue's stale sticky-`visualEffect` hint; laws in navigation.md) and each completed set prints a faint `target:` reference in FLAT ink (ALWAYS when one exists — a line appearing only on a miss would BE the judgment); b6's run de-dupe is HELD BACK, its premise disproved (the stat line has no time; DECISIONS); ⚠️ device pass: month header under the pull, hand-off
