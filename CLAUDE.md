...
- Quick start lives INSIDE Today's sticky week-strip band, under its own QUICK START label (the tally read as captioning the keys — a label claims them; only the non-self-labeling section gets one) and the hairline shelf, with IMPERATIVE keys (`QuickStartLabel`: Run · Ride · Spin · Swim…, collisions fall back to names); ⚠️ device pass: keys must HIT where they draw mid-float (fallback: the below-band row), plus "Spin"/"Swim open water" (Dave ×3)
