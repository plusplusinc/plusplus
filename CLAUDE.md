...
- Today's rail is DATE-FIRST (dates out of cards, node centered on the date row) and quick start is its ANYTIME entry — dashed node + shell `AnytimeCard`, keys MORPH in place into config panels (chrome-only `matchedGeometryEffect` — laws in navigation.md), Train = Start empty / Choose a routine, band = FACTS only; ⚠️ device pass: morph feel, offer-above-plan, the PULL + title at rest (#521's inset class — fallback 8b9e16b), "Spin"/"Swim open water"
