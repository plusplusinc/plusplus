...
- Quick start leaves the timeline for Today's header zone (under the week strip, full width, no spine — an OFFER is not an occurrence) and its keys speak the IMPERATIVE (`QuickStartLabel`: Run · Ride · Spin · Swim…, name-keyed like FormCues, colliding picks fall back to names); ⚠️ device pass: "Spin" and "Swim open water" are flagged judgment calls, and promoting the row INTO the sticky band waits on hit-test proof (Dave, build 159)
