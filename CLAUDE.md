...
**Last updated:** 2026-08-05 · **Latest TestFlight build:** 190, from `claude/search-scopes-control-replace-2z9m5x` (#553, the scope WHEEL restored · the field at the top · the bar yielding on focus) — NOT main, and deliberately so: each is device-passed BEFORE it lands. ⚠️ 170–185 came from parallel sessions and were never recorded here (this line said 169 for three days); 185 was main at `5e94130`, 181–184 feature branches. Read `actions_list`, never this line, for the CURRENT number.
...
**In flight:** #553 — FOUR search-surface changes, as separate commits so each reverts alone. (3) focusing the field HIDES the ++ key and the kit switcher so it rises (`.searchPresentationToolbarBehavior(.automatic)`, reversing build 147) — ✅ device-passed on 188, along with (1)'s scope bar, which disproved the bar-clearing theory. (4) the facet chips' Liquid Glass is REVERTED — it swallowed their taps (interactive glass on a Button's LABEL; the guidance is `.buttonStyle(.glass)`) and its reason dissolved with the native scope bar; the row keeps its evened gaps and the shared `filterChipChrome`. (1) scope is the app's own `InlineWheelPicker` again — RESTORED from #447's ref (⚠️ squash merges can leave a file in no reachable commit while it lives on `refs/pull/<n>/head`; check there before calling anything unrecoverable). Native `.searchScopes` held it for a day, works, and went only because it cannot be STYLED to sit with the filter chips. (2) the FIELD moves to the top (`.navigationBarDrawer(displayMode: .always)`), which cost `Tab(role: .search)` — that role's morph IS the bottom placement (Dave, 2026-08-05). ⚠️ Both can fail outright rather than merely look wrong, which is why builds 186/187 ship from the branch: the revert is a branch that never merged. `LiveWorkoutSettings` (phone's own `HKWorkoutSession`) remains off by default.
