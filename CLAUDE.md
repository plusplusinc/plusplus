...
**Last updated:** 2026-07-08 (evening: Dave's three-task batch — the #224 button pass [PR #225], the #222 equipment sweep to 100 types [PR #226], the #223 routine catalog [PR #227] + the onboarding-test fold fix [PR #228], TestFlight build 31; earlier: builds 29 + 30 — feedback batch #207–#211, search dock #213/#214, polish batch #216, Save kill #219)
...
**plusplus.fit:** LIVE on Vercel (2026-07-08: Dave imported; domains plusplus.fit + www attached, apex 308s to www; marketing page, docs, `/privacy`, and `/r` all serve) — ⚠️ but the project is connected to the **stale private `mrdavidjcole/plusplus.fit`** copy (frozen at pre-transfer 02f5e55), so the live site is three commits behind the canonical `plusplusinc/plusplus.fit`: the LIVE AASA still carries the `TEAMID` placeholder (universal links stay dead until the reconnect — the repo-side fix with the real Team ID WK2XVYGZU9 merged long ago), source links still say mrdavidjcole (redirect, cosmetic), and the MIT license pass isn't deployed. Dave's 1-minute fix: project Settings → Git → connect `plusplusinc/plusplus.fit` (grant the Vercel GitHub App org access if asked); then consider archiving the stale private copy. Tagline is Dave's: "**The hackable workout tracker for incrementing yourself**". The app ships the associated-domains entitlement since build 23, so https share links open the app as soon as the fixed AASA deploys.
...
PlusPlus/                # iOS app target
  ...
  Views/
    Components/               # Shared controls: SearchField, SearchDock (bottom
                              #   glass search + create, #214), SwipeRevealRow +
