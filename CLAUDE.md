...
⚠️ **`ui-test` is RED on main and has been since 2026-07-28** (78fa63d3, dd345e4b, 3e638737, 56083bc9 — four consecutive main pushes), with the same four tests every time: `testSwipeRevealActionSurvivesRelease` (SmokeTests :144), `testRoutinesTabOpensTemplateDetail` (:320, the native search field's Cancel), `testExecuteWorkoutAndSeeHistory` (:484, `"Set 1"` on the record) and `testSetupTimelineOnboarding` (:613, the schedule tray's segment). It is NOT the cardio push — identical on main without it, twice re-run — and NOT a required check, so it never blocked a merge and went unnoticed. ⚠️ The swipe one is the #273/#274 signature that CLAUDE.md recorded as FIXED on 2026-07-15, so treat its return as a regression rather than the old flake. Needs its own investigation; the screenshots artifact is the way in. Build 157 shipped under exactly this condition.
