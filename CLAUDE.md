...
**Last updated:** 2026-08-05 · **Latest TestFlight build:** 186, from `claude/search-scopes-control-replace-2z9m5x` (#553, native search scopes) — NOT main, and deliberately so: the scope bar is device-passed BEFORE it lands. ⚠️ 170–185 came from parallel sessions and were never recorded here (this line said 169 for three days); 185 was main at `5e94130`, 181–184 feature branches. Read `actions_list`, never this line, for the CURRENT number.
...
**In flight:** #553 — the search surface's scope control becomes native `.searchScopes` and the app-drawn `ScopeSegmentedControl` is deleted (Dave, 2026-08-05: another shot at the mechanism retired at builds 140–143). ⚠️ It is UNVALIDATED and can fail outright rather than merely look wrong, which is why build 186 ships from its branch: if the scope bar does not survive on device, the revert is a branch that never merged. `LiveWorkoutSettings` (phone's own `HKWorkoutSession`) remains off by default.
